...
### fix/log-level-cryptopro: CryptoPro barrier логи (2026-04-24)
- **Баг**: `vault/barrier_cryptopro.go:28` — `var cplogger = logging.NewVaultLogger(log.Trace)` — module-level синглтон с захардкоженным Trace-уровнем, игнорировал `log_level` конфига. Остаток отладки.
- **Эффект**: на проде все Debug+Trace из CryptoPro-barrier в лог, включая hex plaintext (под size-guard `<=16`) и base64 ciphertext — под `IsTrace()`-guard-ами, которые при захардкоженном Trace всегда срабатывали.
- **Фикс**: поле `logger log.Logger` в `CryptoProBarrier`, инициализируется прямо в конструкторе тремя строками: `logger := core.Logger().Named("cryptopro-barrier"); core.AddLogger(logger)`. Без хелпера, без nil-fallback — прод-путь единственный (`core_cryptopro.go:122`) и всегда передаёт инициализированный Core. Сигнатуру `NewCryptoProBarrier` НЕ меняли. Импорт `helper/logging` удалён из файла (перестал использоваться).
- **Аналог для seal уже был**: `core_cryptopro.go:149` — `cpWrapper.SetLogger(c.logger.Named("cryptopro-seal"))`. Теперь оба компонента на одном паттерне.
- **`IsTrace()`-guard-ы убраны** по замечанию Игоря: `Trace()` сам проверяет уровень на Info/Debug → строка формируется и дропается до sink. Security OK. Нюанс: на горячем пути Decrypt теперь `base64.StdEncoding.EncodeToString(ciphertext)` считается всегда; если станет дорого — hclog.Fmt с ленивой подстановкой.
- **Tests**: Игорь решил не писать юнит-тесты под эту правку (правка мала и проверяется ручным дым-тестом через `log_level`). Все test-only скаффолдинги из кода убраны. `go build -tags cryptopro` + `go build ./...` зелёные.
- **НЕ трогали**: `barrier_openssl.go:124` (тот же анти-паттерн на Debug, но чужая зона — отдельная задача при желании).
- **Отчёт**: `FIX_log_level_cryptopro.md`.
