...
- **Фикс**: поле `logger log.Logger` в `CryptoProBarrier`, инициализируется из `core.Logger().Named("cryptopro-barrier")` + `core.AddLogger(...)` для `sys/loggers/cryptopro-barrier`. Приватный хелпер `newCryptoProBarrierLogger(core)` (чтобы юнит-тест не требовал HSM). Fallback на `NewVaultLogger(Info)` для `&Core{}` в тестах. Сигнатуру `NewCryptoProBarrier` НЕ меняли.
...
- **Tests**: `vault/barrier_cryptopro_log_test.go` (5 кейсов). `go test ./vault/...` заблокирован pre-existing ESS-1980 build-block (чужая зона). `go build -tags cryptopro` зелёный.
